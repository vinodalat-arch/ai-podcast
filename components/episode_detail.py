...
from utils.content_loader import ARCS, get_related_episodes, get_adjacent_episodes
...
def render_episode_detail(episode):
    ...
    with col1:
        if prev_ep:
            if st.button(
                f"< Ep {prev_ep['episode_number']}: {prev_ep['title'][:30]}...",
                key="prev_ep",
    ...
    with col2:
        if next_ep:
            if st.button(
                f"Ep {next_ep['episode_number']}: {next_ep['title'][:30]}... >",
                key="next_ep",
                use_container_width=True,
            ):
